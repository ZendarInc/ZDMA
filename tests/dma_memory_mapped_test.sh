...
if [ $h2cChannels -gt 0 ]; then
  ...
  do
    ...
    $tool_path/dma_to_device -d /dev/zdma0_h2c_${curChannel} -f data/datafile${i}_4K.bin -s $transferSize -a $addrOffset -c $transferCount &
...
if [ $c2hChannels -gt 0 ]; then
  ...
  do
    ...
    $tool_path/dma_from_device -d /dev/zdma0_c2h_${curChannel} -f data/output_datafile${i}_4K.bin -s $transferSize -a $addrOffset -c $transferCount &
    # If all channels have active transactions we must wait for them to complete
